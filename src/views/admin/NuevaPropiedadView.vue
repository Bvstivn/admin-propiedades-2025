<template>
  <v-card max-width="800" flat class="mx-auto my-10">
    ...
    >
    <v-form class="mt-10">
      <v-text-field class="mb-5" label="Título Propiedad" v-model="titulo.value.value" :error-messages="titulo.errorMessage.value" />
      <v-file-input
        accept="image/*"
      ...
      <v-row>
        <v-col cols="12" md="4">
          <v-select label="Habitaciones" class="mb-5" :items="cantidad" v-model="habitaciones.value.value" :error-messages="habitaciones.errorMessage.value" />
        </v-col>
        <v-col cols="12" md="4">
          <v-select label="Baños" class="mb-5" :items="cantidad" v-model="wc.value.value" :error-messages="wc.errorMessage.value" />
        </v-col>
        <v-col cols="12" md="4">
          <v-select label="Estacionamientos" class="mb-5" :items="cantidad" v-model="estacionamiento.value.value" :error-messages="estacionamiento.errorMessage.value" />
        </v-col>
      </v-row>
